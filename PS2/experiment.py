...
def draw_tl_center(image_in, center, state):
    ...
    """
    output = cv2.rectangle(image_in, (center[0]-1, center[1]-1), (center[0]+1, center[1]+1),
                           (0,0,0), 1)
    output = cv2.putText(output, "(({0},{1}),{2})".format(center[0], center[1], state), 
                         (center[0]+30, center[1]),
                         cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 1.5)
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    output = image_in
    i = 1
    for center, sign in zip(list(signs_dict.values()), list(signs_dict.keys())):
        ...
        i = i + 1
        output = cv2.rectangle(output, (center[0]-1, center[1]-1), (center[0]+1, center[1]+1),
                           (255,0,0), 1)
        output = cv2.putText(output, "('{0}': ({1},{2}))".format(sign, center[0], center[1]),
                             (center[0]-90, center[1]+70+j), cv2.FONT_HERSHEY_SIMPLEX, 
                             0.5, (0,0,0), 1)
