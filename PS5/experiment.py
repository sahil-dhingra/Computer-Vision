...
def part_5():
    ...
    for img in imgs_list:
        ...
        if frame_num==29:
            ...
            kf3 = ps5.KalmanFilter(template_loc3['x'], template_loc3['y'], Q, R)
        ...
        if frame_num in save_frames:
            ...
            if frame_num<60:
                ...
                cv2.rectangle(frame_out, (int(z_x1) - z_w1 // 2, int(z_y1) - z_h1 // 2),
                              (int(z_x1) + z_w1 // 2, int(z_y1) + z_h1 // 2),
                              (0, 0, 255), 2)
            if frame_num<45:
                cv2.circle(frame_out, (int(x2), int(y2)), 10, (255, 0, 0), 2)
                cv2.rectangle(frame_out, (int(z_x2) - z_w2 // 2, int(z_y2) - z_h2 // 2),
                              (int(z_x2) + z_w2 // 2, int(z_y2) + z_h2 // 2),
                              (0, 0, 255), 2)
            if frame_num>=28:
                cv2.circle(frame_out, (int(x3), int(y3)), 10, (255, 0, 0), 2)
                cv2.rectangle(frame_out, (int(z_x3) - z_w3 // 2, int(z_y3) - z_h3 // 2),
...
def part_6():
    ...
    Place all your work in this file and this section.
    """
    template_rect = {'x': 40, 'y': 90, 'w': 160, 'h': 34}
