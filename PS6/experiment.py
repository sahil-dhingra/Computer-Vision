...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    out = np.reshape(x_mean, size)
    result = cv2.resize(out, new_dims) 
...
def part_2a():
    ...
    rand_accuracy = 100*sum(rand_y==ytrain.astype(np.int))/len(ytrain)
    ...
    # TODO: find which of these labels match ytrain and report its accuracy
    wk_accuracy = 100*sum(wk_results==ytrain.astype(np.int))/len(ytrain)
    ...
    rand_accuracy = 100*sum(rand_y==ytest.astype(np.int))/len(ytest)
    print('(Random) Testing accuracy: {0:.2f}%'.format(rand_accuracy))

    # Using Weak Classifier
    wk_results = [wk_clf.predict(x) for x in Xtest]
    # TODO: find which of these labels match ytest and report its accuracy
    wk_accuracy = 100*sum(wk_results==ytest.astype(np.int))/len(ytest)
    print('(Weak) Testing accuracy {0:.2f}%'.format(wk_accuracy))

    y_pred = boost.predict(Xtest)
    # TODO: find which of these labels match ytest and report its accuracy
    boost_accuracy = 100*sum(y_pred==ytest.astype(np.int))/len(ytest)
...
def part_4_a_b():
    ...
    vj_accuracy = 100*sum(predictions==labels)/len(labels)
    ...
    vj_accuracy = 100*sum(predictions==real_labels)/len(real_labels)
